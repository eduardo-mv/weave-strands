...
MeshData MeshBuilder::Build(bool multiBuffer) const
{
	...
	if (multiBuffer) {
		for (auto& attrib : attribs) {
			auto stream = mesh.AllocateStream((uint32_t)types::GetRuntimeTypeTraits(attrib.type).byteSizeVector, attrib.byteSize, attrib.dataToCopy, attrib.isMutable);
		...
		if (index.IsIndexless()) {
			...
			mesh.SetIndex(
				mesh.AllocateIndex(index.type, index.count, index.primitiveRestart, indexToCopy, false));
		}	
	}
	else {
		uint64_t byteSize = 0;
		...
		for (auto& attrib : attribs) {
			...
			mesh.AddAttribute({ attrib.label, ~0u, attrib.type, 0, MeshDataStream{ (uint32_t)types::GetRuntimeTypeTraits(attrib.type).byteSizeVector, bufferView}});
