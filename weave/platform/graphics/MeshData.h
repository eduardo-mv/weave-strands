...
private:
	...
	uint32_t singleSectionLabel{}; 
	uint32_t singleMaterialChannel{};
...
public:
	...
	MeshBuilder& AddIndex(Type const dataToCopy[count], bool primitiveRestart) {
		return AddIndex(types::TypeTraits<Type>::dataTypeValue, uint32_t(count), dataToCopy, primitiveRestart);
	}
